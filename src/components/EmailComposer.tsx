...
export default function EmailComposer({ onSendEmails, isLoading }: EmailComposerProps) {
  ...
  const handleSend = async () => {
    ...
    if (invalidEmails.length > 0) {
      alert(`The following email addresses appear to be invalid: ${invalidEmails.map(r => r.email).join(', ')}`);
      return;
    }
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Tip: Use [Name] in your content to automatically replace it with each recipient's name.
        </p>
      </div>
      ...
      <div>
        ...
        {recipients.length > 0 && (
          <div className="space-y-2">
            <h4 className="font-medium text-gray-700">Added Recipients ({recipients.length})</h4>
      ...
      <div className="pt-4">
        <button
          onClick={handleSend}
          disabled={isLoading || !subject.trim() || !editor?.getHTML().trim() || recipients.length === 0}
        ...
        >
          {isLoading ? 'Sending Emails...' : `Send to ${recipients.length} Recipients`}
        </button>
      </div>
    </div>
  );
